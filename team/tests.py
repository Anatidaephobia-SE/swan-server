...
class TestViews(TestCase):
    ...
    def test_invite_user(self):
        ...
        invite_resp = self.client.post('/api/v1.0.0/team/invite_user', data = {'team_url' : 'test_url', 'username' : 'amir.j1882@gmail.com'}, **{'HTTP_Authorization' : 'bear ' + self.token})
        assert invite_resp.status_code == 404

        create_team_resp = self.client.post('/api/v1.0.0/team/create_team', data = {'name' : 'test_name', 'url' : 'test_url'}, **{'HTTP_Authorization' : 'bear ' + self.token})

        invite_resp2 = self.client.post('/api/v1.0.0/team/invite_user', data = {'team_url' : 'test_url', 'username' : 'amir.j1882@gmail.com'}, **{'HTTP_Authorization' : 'bear ' + self.token})

        assert invite_resp2.status_code == 200

        
        invite_resp3 = self.client.post('/api/v1.0.0/team/invite_user', data = {'team_url' : 'test_url', 'username' : 'nothing'}, **{'HTTP_Authorization' : 'bear ' + self.token})
